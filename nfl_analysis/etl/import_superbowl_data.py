...
def import_superbowl_data():
    ...
    all_superbowl_tables = utils.get_wiki_table(
        "https://de.wikipedia.org/wiki/Super_Bowl", first=False
    )
    afl_championships = all_superbowl_tables[0]
    superbowls_nfl = all_superbowl_tables[1]
    superbowl_stats = all_superbowl_tables[2]

    afl_championships = afl_championships.rename(
        {"Siegende Liga": "Siegende Conference", "MVP *": "MVP"}, axis=1
    )
    ...
    superbowls[["overtime", "points_winner", "points_looser"]] = (
        superbowls["ergebnis"].apply(utils.parse_result).to_list()
    )
    # Correct hidden zeros
    superbowls.loc[
        superbowls["points_winner"] < superbowls["points_looser"], "points_looser"
    ] /= 10
    superbowls["total_points"] = (
        superbowls["points_winner"] + superbowls["points_looser"]
    )
