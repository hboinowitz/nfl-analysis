...
def parse_result(result: str) -> Tuple[int, int, bool]:
    ...
    if len(splitted_result) > 2:
        return ValueError(
            "The `result` you have passed does not match the reuiqred format."
        )
    if len(splitted_result) == 2:
...
remove_brackets = lambda string_with_brakets: re.sub(
    r"\[.*?\]", "", string_with_brakets
)
...
def team_names_to_mapping(team_names: pd.Series) -> pd.DataFrame:
    ...
    team_names[["current_name", "historic_name"]] = (
        team_names["Team"].apply(parse_historic_team_names).to_list()
    )
