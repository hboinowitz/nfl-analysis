...
def plot_wins_and_losses_per_division(
    wins_and_losses_per_division_cumulated: pd.DataFrame, league: str, division: str
):
...
def add_current_team_names_to_superbowl_data(
    superbowl: pd.DataFrame, mapping: pd.DataFrame
):
    superbowls_with_current_team_names = (
        superbowl.merge(mapping, left_on="sieger", right_on="historic_name")
        .merge(
            mapping,
            left_on="verlierer",
...
def add_divisions_to_superbowl_data(
    superbowl: pd.DataFrame, mapping: pd.DataFrame, teams: pd.DataFrame
):
    mapping_club_to_divison = get_mapping_club_to_division(teams)
    superbowls_nfl_with_current_team_names = add_current_team_names_to_superbowl_data(
        superbowl, mapping
    )

    superbowls_nfl_with_divisions = (
        superbowls_nfl_with_current_team_names.merge(
            mapping_club_to_divison,
...
def seperate_looser_and_winner_frame(base_frame: pd.DataFrame, indicator: str):
    seperated_frame = base_frame[
        ["datum", f"league_{indicator}", f"division_{indicator}"]
    ]
    remove_indicator = lambda str_with_indicator: str_with_indicator.replace(
        f"_{indicator}", ""
    )
...
def expand_superbowl_data_vertically(
    superbowl: pd.DataFrame, mapping: pd.DataFrame, teams: pd.DataFrame
):
    superbowls_nfl_with_divisions = add_divisions_to_superbowl_data(
        superbowl, mapping, teams
    )
    superbowls_nfl_with_divisions = superbowls_nfl_with_divisions.drop_duplicates(
        "datum"
    )
...
def get_cumulated_wins_per_division(
    superbowl: pd.DataFrame, mapping: pd.DataFrame, teams: pd.DataFrame
):
    combined_wins_and_losses = expand_superbowl_data_vertically(
        superbowl, mapping, teams
    )
    wins_and_losses_per_division_cumulated = (
        combined_wins_and_losses.pivot(
            index="datum", columns=["league", "division"], values=["winner", "looser"]
        )
        .fillna(0)
        .cumsum()
    )
